...
/**
 * An integration test class that holds all JDBC sink pipeline integration tests for Snowflake.
 *
 ...
 *
 * <p>NOTE: Several methods have been modified to support Snowflake specific data types and behavior.
 *
...
public abstract class AbstractJdbcSinkPipelineIT extends AbstractJdbcSinkIT {
        ...
        assertDataType(source,
                ...
                (rs, index) -> {
                    switch (sink.getType()) {
                        ...
                        case SNOWFLAKE:
                            // storing it as varchar representing the bit string
                            return new BigInteger(rs.getString(index), 2).intValue();
                        default:
    ...
    public void testBitWithSizeDataTypeNotInKey(Source source, Sink sink) throws Exception {
        ...
        sink.assertRows(getSinkTable(record, sink), rs -> {
            // snowflake jdbc driver does not support getBlob
            // final Blob blob = rs.getBlob(1);
            final String string = rs.getString(1);
            final BigInteger bigint = new BigInteger(string, 2);
            final byte[] byteArray = bigint.toByteArray();
            assertThat(byteArray).isEqualTo(ByteBuffer.allocate(1).put((byte) 1).array());
            return null;
        });
    }
    ...
    public void testNumberWithPrecisionAndScaleDataType(Source source, Sink sink) throws Exception {
        assertDataTypes(source,
                ...
                (record) -> {
                    if (SinkType.SNOWFLAKE.is(sink.getType())) {
                        assertColumn(sink, record, "id0", "NUMBER", 2, 1);
                        assertColumn(sink, record, "id1", "NUMBER", 3, 1);
                        assertColumn(sink, record, "id2", "NUMBER", 8, 1);
                        assertColumn(sink, record, "id3", "NUMBER", 18, 1);
                        assertColumn(sink, record, "id4", "NUMBER", 24, 1);
                        assertColumn(sink, record, "data0", "NUMBER", 2, 1);
                        assertColumn(sink, record, "data1", "NUMBER", 3, 1);
                        assertColumn(sink, record, "data2", "NUMBER", 8, 1);
                        assertColumn(sink, record, "data3", "NUMBER", 18, 1);
                        assertColumn(sink, record, "data4", "NUMBER", 24, 1);
                    }
                    else {
    ...
    public void testNumberWithPrecisionAndNegativeScaleDataType(Source source, Sink sink) throws Exception {
        ...
        assertDataTypes(source,
                ...
                (record) -> {
                    ...
                    else if (SinkType.SNOWFLAKE.is(sink.getType())) {
                        assertColumn(sink, record, "id3", "NUMBER", 18, 0);
                        assertColumn(sink, record, "id4", "NUMBER", 24, 0);
                        assertColumn(sink, record, "data3", "NUMBER", 18, 0);
                        assertColumn(sink, record, "data4", "NUMBER", 24, 0);
                    }
                    else {
        ...
        assertDataType(source,
                ...
                (record) -> {
                    ...
                    if (source.getType().is(SourceType.POSTGRES)) {
                        assertColumn(sink, record, "id", "DOUBLE");
                        assertColumn(sink, record, "data", "DOUBLE");
                    }
                    else if (SinkType.SNOWFLAKE.is(sink.getType())) {
                        assertColumn(sink, record, "id", "NUMBER");
                        assertColumn(sink, record, "data", "NUMBER");
                    }
                    else {
        ...
        assertDataTypes(source,
                ...
                (record) -> {
                    ...
                    if (SourceType.ORACLE.is(source.getType())) {
                        ...
                        assertColumn(sink, record, "id0", "NUMBER", 2, 0);
                        assertColumn(sink, record, "id1", "NUMBER", 3, 0);
                        assertColumn(sink, record, "id2", "NUMBER", 8, 0);
                        assertColumn(sink, record, "id3", "NUMBER", 18, 0);
                        assertColumn(sink, record, "id4", "NUMBER", 24, 0);
                        assertColumn(sink, record, "data0", "NUMBER", 2, 0);
                        assertColumn(sink, record, "data1", "NUMBER", 3, 0);
                        assertColumn(sink, record, "data2", "NUMBER", 8, 0);
                        assertColumn(sink, record, "data3", "NUMBER", 18, 0);
                        assertColumn(sink, record, "data4", "NUMBER", 24, 0);
                    }
                    else {
    ...
    public void testDecimalDataType(Source source, Sink sink) throws Exception {
        assertDataType(source,
                ...
                (record) -> {
                    // Precision assertion is skipped, varies by the source.
                    if (sink.getType().is(SinkType.SNOWFLAKE)) {
                        // snowflake creates them as double precision but stores them as float (an internal synonym)
                        assertColumn(sink, record, "id", "DOUBLE");
                        assertColumn(sink, record, "data", "DOUBLE");
                    }
                    else if (source.getType().is(SourceType.POSTGRES)) {
                        assertColumn(sink, record, "id", getVariableScaleDecimalType());
                        assertColumn(sink, record, "data", getVariableScaleDecimalType());
                    }
                    else {
    ...
    public void testDecimalWithPrecisionDataType(Source source, Sink sink) throws Exception {
        assertDataTypes(source,
                ...
                (record) -> {
                    ...
                    // for precision >= 19.
                    else if (SourceType.ORACLE.is(source.getType())) {
                        assertColumn(sink, record, "id0", getInt8Type());
                        assertColumn(sink, record, "id1", getInt16Type());
                        assertColumn(sink, record, "id2", getInt32Type());
                        assertColumn(sink, record, "id3", getInt64Type());
    ...
    public void testRealDataType(Source source, Sink sink) throws Exception {
        assertDataType(source,
                ...
                (record) -> {
                    if (sink.getType().is(SinkType.SNOWFLAKE)) {
                        // snowflake will issue double create but actually store as a float
                        assertColumn(sink, record, "id", "DOUBLE");
                        assertColumn(sink, record, "data", "DOUBLE");
                    }
                    else {
    ...
    public void testFloatDataType(Source source, Sink sink) throws Exception {
        assertDataType(source,
                ...
                (record) -> {
                    if (sink.getType().is(SinkType.SNOWFLAKE)) {
                        // snowflake issues create double precision but stores as a float
                        assertColumn(sink, record, "id", "DOUBLE");
                        assertColumn(sink, record, "data", "DOUBLE");
                    }
                    else {
                ...
                (record) -> {
                    ...
                    switch (sink.getType()) {
                        ...
                        case SNOWFLAKE:
                            ...
                            assertColumn(sink, record, "data", "VARCHAR");
                            break;
                    }
                },
                ResultSet::getString);
    ...
    public void testTimeDataType(Source source, Sink sink) throws Exception {
        ...
        assertDataType(source,
                ...
                List.of(OffsetTime.of(1, 2, 3, nanoSeconds, ZoneOffset.UTC),
                ...
                /* snowflake's TIME is zone un-aware and can only do timezone utc */
                this::getTimeAtUTCOffsetTime);
    }
    ...
    public void testTimeWithPrecisionDataType(Source source, Sink sink) throws Exception {
        ...
        final List<OffsetTime> expectedValues = List.of(
                OffsetTime.of(1, 2, 3, nanoSeconds0, ZoneOffset.UTC),
                OffsetTime.of(14, 15, 16, nanoSeconds1, ZoneOffset.UTC),
                OffsetTime.of(1, 2, 3, nanoSeconds0, ZoneOffset.UTC),
                OffsetTime.of(14, 15, 16, nanoSeconds1, ZoneOffset.UTC));
    ...
    public void testIntervalDataTypeIntervalHandlingModeNumeric(Source source, Sink sink) throws Exception {
        ...
        else if (sink.getType().is(SinkType.SNOWFLAKE)) {
            assertDataTypeNonKeyOnly(source,
                    sink,
                    "interval",
                    List.of("'P1Y2M3DT4H5M6.78S'::INTERVAL"),
                    ...
                    (config) -> config.with("interval.handling.mode", "numeric"),
                    (record) -> assertColumn(sink, record, "data", "NUMBER"),
                    ResultSet::getLong);
        }
        else {
            assertDataTypeNonKeyOnly(source,
    ...
    public void testByteaDataType(Source source, Sink sink) throws Exception {
        if (sink.getType().is(SinkType.SNOWFLAKE)) {
            assertDataType(source,
                    ...
                    (record) -> {
                        assertColumn(sink, record, "id", "BINARY");
                        assertColumn(sink, record, "data", "BINARY");
                    },
                    ResultSet::getBytes);
    ...
    protected OffsetTime getTimeAtUTCOffsetTime(ResultSet rs, int index) throws SQLException {
        LOGGER.trace(getTimestamp(rs, index) + " " + getTimestamp(rs, index).getNanos());
        java.sql.Timestamp timestamp = getTimestamp(rs, index);
        LOGGER.trace(timestamp.toString());
        java.time.Instant instant = timestamp.toInstant();
        LOGGER.trace(instant.toString());
        java.time.ZonedDateTime zonedDateTime = instant.atZone(ZoneId.of("UTC"));
        java.time.LocalTime localTime = zonedDateTime.toLocalTime();
        LOGGER.trace(localTime.toString());
    ...
    protected <U> void consumeAndAssert(Sink sink, DataTypeColumnAssert columnAssert, List<U> expectedValues, ColumnReader<U> columnReader) throws Exception {
        ...
        // NOTE: Snowflake test does not support querying the database state, since a testcontainer is not used
        // if (TestHelper.shouldQueryDatabaseState()) {
